...
# val_output_folder = '/wdata/dpn92_9ch_val_0'

speed_bins = [15, 18.75, 20, 25, 30, 35, 45, 55, 65]

cities = [('AOI_7_Moscow', '/data/SN5_roads/train/AOI_7_Moscow', 'train_AOI_7_Moscow_geojson_roads_speed_wkt_weighted_simp.csv'),
          ('AOI_8_Mumbai', '/data/SN5_roads/train/AOI_8_Mumbai', 'train_AOI_8_Mumbai_geojson_roads_speed_wkt_weighted_simp.csv'),
          ('AOI_2_Vegas', '/data/SN3_roads/train/AOI_2_Vegas', 'train_AOI_2_Vegas_geojson_roads_speed_wkt_weighted_simp.csv'),
          ('AOI_3_Paris', '/data/SN3_roads/train/AOI_3_Paris', 'train_AOI_3_Paris_geojson_roads_speed_wkt_weighted_simp.csv'),
          ('AOI_4_Shanghai', '/data/SN3_roads/train/AOI_4_Shanghai', 'train_AOI_4_Shanghai_geojson_roads_speed_wkt_weighted_simp.csv'),
          ('AOI_5_Khartoum', '/data/SN3_roads/train/AOI_5_Khartoum', 'train_AOI_5_Khartoum_geojson_roads_speed_wkt_weighted_simp.csv')]
...
if __name__ == '__main__':
    ...
    snapshot_name = 'dpn92_9ch_{}_1'.format(seed)
    ...
    for i in train_idxs0:
        train_idxs.append(i)
        if (('Mumbai' in train_files[i]) or ('Moscow' in train_files[i])) and random.random() > 0.5:
            train_idxs.append(i)
        if (('Paris' in train_files[i]) or ('Khartoum' in train_files[i])) and random.random() > 0.1:
            train_idxs.append(i)
    ...
    model = Dpn92_9ch_Unet(pretrained=False) #.cuda()

    params = model.parameters()

    optimizer = AdamW(params, lr=0.0001, weight_decay=1e-5) 
    
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[4, 8, 12, 16, 18, 20], gamma=0.5)

    model = nn.DataParallel(model).cuda()
    
    snap_to_load = 'dpn92_9ch_{}_0_best'.format(seed)
