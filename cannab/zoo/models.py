...
class SeResNext50_Unet(nn.Module):
    def __init__(self, pretrained='imagenet', **kwargs):
        super(SeResNext50_Unet, self).__init__()
        
        encoder_filters = [64, 256, 512, 1024, 2048]
        decoder_filters = np.asarray([64, 96, 128, 256, 512])
        ...
        self.res = nn.Conv2d(decoder_filters[-5], 13, 1, stride=1, padding=0)
        ...
        self.conv1 = nn.Sequential(encoder.layer0.conv1, encoder.layer0.bn1, encoder.layer0.relu1)
...
class SeResNext50_Unet_9ch(nn.Module):
    def __init__(self, pretrained='imagenet', **kwargs):
        super(SeResNext50_Unet_9ch, self).__init__()
        ...
        conv1_new = nn.Conv2d(9, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        _w = encoder.layer0.conv1.state_dict()
        _w['weight'] = torch.cat([0.5 * _w['weight'], 0.25 * _w['weight'], 0.25 * _w['weight']], 1)
        conv1_new.load_state_dict(_w)
...
class Res34_Unet(nn.Module):
    def __init__(self, pretrained=True, **kwargs):
        ...
        decoder_filters = np.asarray([64, 64, 128, 256, 512])
...
class Res34_9ch_Unet(nn.Module):
    def __init__(self, pretrained=True, **kwargs):
        super(Res34_9ch_Unet, self).__init__()
