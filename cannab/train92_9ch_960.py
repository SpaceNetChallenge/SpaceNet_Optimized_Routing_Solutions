...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (('Moscow' not in img_id) and ('Mumbai' not in img_id) and (random.random() > 0.5)) or (random.random() > 0.7):
            img = img[::-1, ...]
            img2 = img2[::-1, ...]
            img3 = img3[::-1, ...]
            msk0 = msk0[::-1, ...]
        ...
        if ('Moscow' not in img_id) and ('Mumbai' not in img_id) or random.random() > 0.6:
...
if __name__ == '__main__':
    ...
    snapshot_name = 'dpn92_9ch_{}_0'.format(seed)
    ...
    for i in train_idxs0:
        train_idxs.append(i)
        if (('Mumbai' in train_files[i]) or ('Moscow' in train_files[i])) and random.random() > 0.1:
            train_idxs.append(i)
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[4, 10, 16, 24, 30, 35, 40, 44, 47], gamma=0.5)
