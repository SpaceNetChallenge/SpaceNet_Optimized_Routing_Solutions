...
def wkt_to_G(wkt_list,
    ...
    if verbose:
        logger.info("Number of nodes: {}".format(len(Gout.nodes())))
        logger.info("Number of edges: {}".format(len(Gout.edges())))
...
def cleaning_graph(conf):
    ...
    # outut files
    path_images = conf.test_data_refined_dir_ims

    # outut csv file
    csv_file = "{}/working/sp5r2/models/wkt/{}/wkt_submission_nospeed.csv".format(
    ...
    min_spur_length_m = conf.min_spur_length_m
    ...
    with Pool(cpu_count()) as p:
        data = list(tqdm.tqdm(
            iterable=p.imap_unordered(_cleaning_graph_iterable, args_list),
            total=len(args_list)))
...
def _cleaning_graph_iterable(args):
    (
        i,
        image_id,
        graph_dir,
    ...
    if verbose:
        logger.info("Time to create graph: {} seconds".format(t2-t1))
