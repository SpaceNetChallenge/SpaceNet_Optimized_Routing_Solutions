...
class Estimator:
    ...
    def calculate_loss_single_channel(self, output, target, meter, training, 
        ...
        #print('\ntarget.shape, output.shape', target.shape, output.shape)

        #print('output mean, max, min', output.mean(), output.max(), output.min())
        ...
        focal_c = focal_cannab(output, target)
        ...
        if 'focal' in self.config.loss.keys():
            loss += self.config.loss['focal'] * focal_l / iter_size
            #loss = (self.config.loss['focal'] * focal_l + self.config.loss['soft_dice'] * dice_soft_l ) / iter_size
        if 'bce' in self.config.loss.keys():
            loss += self.config.loss['bce'] * bce / iter_size
        if 'focal_cannab' in self.config.loss.keys():
            loss += self.config.loss['focal_cannab'] * focal_c / iter_size
        if 'smooth_l1' in self.config.loss.keys():
            loss += self.config.loss['smooth_l1'] * smooth_l1_l / iter_size
        if 'mse' in self.config.loss.keys():
            loss += self.config.loss['mse'] * mse_l / iter_size
        #if 'ce' in self.config.loss.keys():
        #    loss += self.config.loss['ce'] * ce / iter_size
        if 'dice' in self.config.loss.keys():
            loss += self.config.loss['dice'] * dice_l / iter_size
        if 'soft_dice' in self.config.loss.keys():
            loss += self.config.loss['soft_dice'] * dice_soft_l / iter_size
        ...
        meter['focal_cannab'] = focal_c.data.cpu().numpy()[0] / iter_size
        ...
        meter['soft_dice_l'] = dice_soft_l.data.cpu().numpy()[0] / iter_size
...
class PytorchTrain:
    ...
    def __init__(self, estimator: Estimator, fold, callbacks=None, hard_negative_miner=None):
        ...
        self.estimator.resume("checkpoint.pth")
...
def train(ds, fold, train_idx, val_idx, config, save_path, log_path, 
    ...
    try:
        model = torch.load(config.save_weights_dir)
    except:
    ...
    callbacks = [
        ModelSaver(1, ("best.pth"), best_only=True),
        ModelSaver(1, ("last.pth"), best_only=False),
        CheckpointSaver(1, ("checkpoint.pth")),
    ...
    print('len ds:', len(ds))
    print(len(TrainDataset(ds, train_idx, config, transforms=transforms)))
