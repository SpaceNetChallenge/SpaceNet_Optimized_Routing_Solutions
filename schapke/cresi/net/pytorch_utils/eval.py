...
def read_model(path_model_weights):
    ...
    with warnings.catch_warnings():
        ...
        model = torch.load(os.path.join(path_model_weights, 'best.pth'))
        model.eval()
...
class Evaluator:
    ...
    def predict(self, fold, val_indexes, weight_dir, verbose=False):
        ...
        prefix = "" if not fold or not self.test else ('fold' + str(fold) + "_")
        print ("prefix:", prefix)
        ...
        for data in pbar:
            ...
            if verbose:
                ...
                print("  eval.py - Evaluator - predict() - predicted.mean:", predicted.mean())
                print("  eval.py - Evaluator - predict() - predicted.max:", predicted.max())
