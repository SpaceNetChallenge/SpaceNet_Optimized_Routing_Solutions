...
from torch.nn import UpsamplingNearest2d, Conv2d, Sequential, UpsamplingBilinear2d
    ...
    def __init__(self, node_kernel, out_dim, channels, up_factors):
        ...
        for i, c in enumerate(channels):
            ...
            if f == 1:
                ...
                #todo:
                up = Sequential(nn.Upsample(scale_factor=2),
...
class DLASeg(nn.Module):
    ...
    def optim_parameters(self, memo=None):
        for param in self.encoder_stages.parameters():
            yield param
...
def dla34up(classes, pretrained_base='imagenet', **kwargs):
    model = DLASeg('dla34', classes, pretrained_base=pretrained_base, **kwargs)
    return model


def dla60up(classes, pretrained_base='imagenet', **kwargs):
    model = DLASeg('dla60', classes, pretrained_base=pretrained_base, **kwargs)
    return model


def dla102up(classes, pretrained_base='imagenet', **kwargs):
    model = DLASeg('dla102', classes,
                   pretrained_base=pretrained_base, **kwargs)
    return model


def dla169up(classes, pretrained_base='imagenet', **kwargs):
    model = DLASeg('dla169', classes,
                   pretrained_base=pretrained_base, **kwargs)
    return model

if __name__ == '__main__':
    d = dla102up(8, pretrained_base="imagenet")
    with torch.no_grad():
        ret = d(torch.rand(8, 3, 512, 512))
        print(ret.size())