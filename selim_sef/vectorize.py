...
from tools.sknw import build_sknw
import warnings
...
def make_skeleton(root, fn, debug, threshes, fix_borders):
    ...
    if img.shape[0] > 1300:
        pady = (img.shape[0] - 1300)//2
        padx = (img.shape[1] - 1300)//2
        img = img[pady:-pady, padx:-padx]
...
def add_small_segments(G, terminal_points, terminal_lines):
    ...
    dists = OrderedDict(sorted(dists, key=lambda x: x[1]))
...
def build_graph(root, fn, debug=False, threshes={'2': .3, '3': .3, '4': .3, '5': .2}, add_small=True, fix_borders=True):
    ...
    city = city.replace("SN3_roads_train_", "").replace("SN5_roads_train_", "")
...
def vectorize_dir(root, output_path):
    ...
    with Pool(processes=14) as p:
        with tqdm.tqdm(total=len(l)) as pbar:
            for i, v in tqdm.tqdm(enumerate(p.imap_unordered(f, l))):
                pbar.update()
                data.append(v)
    all_data = []
    for k, v in data:
        for val in v:
            all_data.append((k.replace("PS-RGB_", ""), val, 0, 1))
    df = pd.DataFrame(all_data, columns=['ImageId', 'WKT_Pix', "dich", "dich"])
