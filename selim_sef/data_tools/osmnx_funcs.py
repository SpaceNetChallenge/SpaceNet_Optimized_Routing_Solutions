...
def project_gdf(gdf, to_crs=None, to_latlong=False, verbose=False):
    ...
    # determine UTM zone
        ...
        if to_latlong:
            ...
            if verbose:
                print('Projected the GeoDataFrame "{}" to default_crs in {:,.2f} seconds'.format(gdf.gdf_name,
            ...
            if verbose:
                print('Projected the GeoDataFrame "{}" to UTM-{} in {:,.2f} seconds'.format(gdf.gdf_name, utm_zone,
                                                                                            time.time() - start_time))
...
def project_graph(G, to_crs=None, verbose=False):
    ...
    if verbose:
        print(
            'Extracted projected node geometries from GeoDataFrame in {:,.2f} seconds'.format(time.time() - start_time))
...
def graph_to_gdfs(G, nodes=True, edges=True, node_geometry=True,
    ...
    if nodes:
        ...
        if verbose:
            print('Created GeoDataFrame "{}" from graph in {:,.2f} seconds'.format(gdf_nodes.gdf_name,
                                                                                   time.time() - start_time))
    ...
    if edges:
        ...
        if verbose:
            print('Created GeoDataFrame "{}" from graph in {:,.2f} seconds'.format(gdf_edges.gdf_name,
                                                                                   time.time() - start_time))
    ...
    if save:
        ...
        if file_format == 'svg':
            ...
            if axis_off:
                ...
                extent = 'tight'
            if len(filename) > 0:
                fig.savefig(path_filename, dpi=dpi, bbox_inches=extent, format=file_format,
...
def simplify_graph(G, strict=True, verbose=False):
    ...
    if verbose:
        print(msg.format(initial_node_count, len(list(G.nodes())), initial_edge_count, len(list(G.edges())),
                         time.time() - start_time))
    return G
