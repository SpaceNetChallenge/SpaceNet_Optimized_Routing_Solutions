...
def add_edges(G, dist3=25, boarder=40):
    points = G.nodes()
    if G is None or len(G.nodes()) == 0:
        return
    node = G.node
    term = [node[t]['o'] for t in G.nodes()]

    term_points = [node for node in G.nodes]

    counter = max(G.nodes()) + 1
    for i, t1 in enumerate(term_points):
        p1 = node[t1]['o']
        for j, t2 in enumerate(term_points):
            p2 = node[t2]['o']
            if j <= i:
                continue
            if not (G.has_edge(t1, t2) or G.has_edge(t2, t1)):
                continue
            for k, t3 in enumerate(term_points):
                if k == j or k == i:
                    continue

                super_verb = False
                if t1 in range(51, 61) and t2 in range(51, 61) and t3 in range(51, 61):
                    super_verb = True

                p3 = node[t3]['o']

                if (p3 < boarder).any() or (1300 - p3 < boarder).any():
                    continue

                if G.has_edge(t1, t3) or G.has_edge(t2, t3) or G.has_edge(t3, t2) or G.has_edge(t3, t1):
                    continue

                if G.has_edge(t1, t2) and len(dict(G[t1][t2])) > 1:
                    continue

                if G.has_edge(t2, t1) and len(dict(G[t2][t1])) > 1:
                    continue

                p1_dist = np.linalg.norm(p1 - p3)
                p2_dist = np.linalg.norm(p2 - p3)

                if G.has_edge(t1, t2):
                    edge = dict(G[t1][t2])[0]
                elif G.has_edge(t2, t1):
                    edge = dict(G[t2][t1])[0]
                else:
                    continue
                ...
                if 'pts' in edge.keys():
                    ...
                    pts_num = np.argmin(np.linalg.norm(edge['pts'] - p3, axis=1))
                    cur_dist = np.linalg.norm(edge['pts'][pts_num] - p3)

                    if cur_dist > dist3:
                        continue

                    if p1_dist <= cur_dist or p2_dist <= cur_dist:
                        # continue
                        # pts_to_skip = [[239, 644]]
                        # found = False
                        # for pts_to in pts_to_skip:
                        #     if p1[0] == pts_to[0] and p1[1] == pts_to[1]:
                        # print('SKIPPING', p1, p2, p3)
                        # print('THEIR T', t1, t2, t3)
                        # found = True
                        # break
                        # if found:
                        #     print('BEFORE')
                        #     print(G.has_edge(t1, t2), G.has_edge(t2, t1))
                        #     print('NEWDEGREE', dict(G.degree())[t1], dict(G.degree())[t2], G.degree()[t3])
                        #     print(dict(G[t2][t1]))
                        #     print(dict(G[t1][t2]))

                        # print('EDGE ADDED TO ANOTHER')
                        # print(p1, p2, p3)
                        # print(p1_dist, p2_dist)
                        if p1_dist < p2_dist:
                            distp3 = np.linalg.norm(p1 - p3)

                            zeros = np.zeros((1300, 1300, 3))
                            cv2.line(zeros, tuple(p1), tuple(p3), (0, 255, 0), 1)
                            zeros = np.max(zeros, axis=2)
                            pts_to_add = np.argwhere(zeros > 0)
                            pts_to_add = pts_to_add[1:-1, ::-1]
                            G.add_edge(t1, t3, weight=distp3, pts=pts_to_add)
                            # G.add_edge(t3, t1, weight=distp3)
                        else:

                            distp3 = np.linalg.norm(p2 - p3)
                            zeros = np.zeros((1300, 1300, 3))
                            cv2.line(zeros, tuple(p2), tuple(p3), (0, 255, 0), 1)
                            zeros = np.max(zeros, axis=2)
                            pts_to_add = np.argwhere(zeros > 0)
                            pts_to_add = pts_to_add[1:-1, ::-1]
                            G.add_edge(t2, t3, weight=distp3, pts=pts_to_add)
                            # G.add_edge(t3, t2, weight=distp3)
                            # if found:
                            # print('AFTER')
                            # print(G.has_edge(t1, t2), G.has_edge(t2, t1))
                            # print('NEWDEGREE', dict(G.degree())[t1], dict(G.degree())[t2], G.degree()[t3])
                        ...
                        G.add_edge(t3, counter)
                        print('Added', p3, a)
                        G.add_edge(t1, counter, pts=edge['pts'][:pts_num],
                                   weight=pts_num / len(edge['pts']) * edge['weight'])
                        print('Added', p1, a)
                        G.add_edge(t2, counter, pts=edge['pts'][pts_num:],
                                   weight=(1 - pts_num / len(edge['pts'])) * edge['weight'])
                        print('Added', p2, a)
                        print('DEGREE NOW', G.degree()[t3])
                        counter += 1
                    ...
                    # if super_verb:
                    # print("PTS NOT IN KEYS")
                    lamb = np.dot(p3 - p2, p1 - p2) / np.dot(p1 - p2, p1 - p2)
                    if lamb <= 0 or lamb >= 1:
                        cur_dist = min(np.linalg.norm(p3 - p2), np.linalg.norm(p3 - p1))

                    else:
                        cur_dist = np.linalg.norm(np.cross(p2 - p1, p1 - p3)) / np.linalg.norm(p2 - p1)

                    if cur_dist > dist3:
                        continue
                    else:
                        # print('dist', cur_dist)
                        # print('Has edge?', p1, p2, G.has_edge(t1, t2), G.has_edge(t2, t1))
                        # print(G.edges.data())
                        if 0 < lamb < 1:
                            print(p1, p2, p3)
                            a = lamb * (p1 - p2) + p2
                            ...
                            G.add_edge(t3, counter)
                            print('Added', p3, a)
                            G.add_edge(t1, counter)
                            print('Added', p1, a)
                            G.add_edge(t2, counter)
                            print('Added', p2, a)
                            # print('DEGREE NOW', G.degree()[t3])
                            counter += 1
                        else:
                            if np.linalg.norm(p3 - p2) < np.linalg.norm(p3 - p1):
                                G.add_edge(t2, t3)
                            else:
                                G.add_edge(t1, t3)
...
###############################################################################
def add_small_segments(G, terminal_points, terminal_lines, dist1=20,  # default 20 and 100
                       dist2=100, angle1=20, angle2=160):
    node = G.node
...
def build_graph_wkt(img_loc, out_ske_file, out_gpickle='', thresh=0.3,
                    ...
                    verbose=False):
    # create skeleton
    img_copy, ske = make_skeleton(img_loc, thresh, debug, fix_borders,
    ...
    for i, w in enumerate(node_lines):
        ...
        for s, e in pairwise(w):
            ...
            for ix, val in enumerate(vals):
                ...
                # if s == 43 and e == 45:
                # print('LINESTRINGS', line_strings)
                if ix == 0:
...
def build_wkt_imfile(imfile, nfiles, indir, outfile, out_ske_dir, out_gdir='', thresh=0.3,
                     # threshes={'2': .3, '3': .3, '4': .3, '5': .2},
                     im_prefix='',
                     debug=False, add_small=True, fix_borders=True,
                     img_shape=(1300, 1300),
    ...
    t1 = time.time()
    print("\n\n", 1, "/", nfiles, ":", imfile)
    logger1.info("{x} / {y} : {z}".format(x=1, y=nfiles, z=imfile))
