...
def read_model(path_model_weights, fold):
    ...
    with warnings.catch_warnings():
        ...
        model_pepeg = torch.load(os.path.join(path_model_weights, 'fold{}_best.pth'.format(fold)))
        model = fpn.resnet50_fpn(num_classes=8, num_channels=3)
        model.load_state_dict(model_pepeg.state_dict(), strict=False)
...
class Evaluator:
    ...
    def predict(self, fold, val_indexes, weight_dir, verbose=False):
        ...
        val_dataset = SequentialDataset(self.ds, val_indexes, stage='test', config=self.config,
                                        transforms=self.val_transforms)
        val_dl = PytorchDataLoader(val_dataset, batch_size=self.config.predict_batch_size, num_workers=self.num_workers,
                                   drop_last=False)
