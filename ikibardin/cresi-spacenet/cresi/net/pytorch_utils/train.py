...
from pytorch_zoo import unet, fpn
...
class Estimator:
    ...
    def resume(self, checkpoint_name):
        ...
        print("resumed from checkpoint {} on epoch: {}".format(os.path.join(self.save_path, checkpoint_name),
                                                               self.start_epoch))
        return True
    ...
    def calculate_loss_single_channel(self, output, target, meter, training,
        ...
        if 'focal' in self.config.loss.keys():
            ...
            loss = (self.config.loss['focal_cannab'] * focal_l + self.config.loss[
                'soft_dice'] * dice_soft_l) / iter_size
        ...
        # meter['dice_round'] += dice_r.data.cpu().numpy() / iter_size
        # meter['jr'] += jacc_r.data.cpu().numpy() / iter_size
        # meter['jacc'] += jacc.data.cpu().numpy() / iter_size
        # meter['dice'] += d.data.cpu().numpy() / iter_size
...
class PytorchTrain:
    ...
    def _run_one_epoch(self, epoch, loader, training=True):
        ...
        pbar = tqdm(enumerate(loader), total=len(loader),
                    desc="Fold {}; Epoch {}{}".format(self.fold, epoch, ' eval' if not training else ""), ncols=0)
        for i, data in pbar:
...
def train(ds, fold, train_idx, val_idx, config, save_path, log_path,
    ...
    val_loader = PytorchDataLoader(
        ValDataset(val_ds if val_ds is not None else ds, val_idx, config, transforms=val_transforms),
