...
def wkt_to_G(wkt_list, im_file=None, min_subgraph_length_pix=30,
    ...
    nodes_edges = wkt_list_to_nodes_edges(wkt_list)
    if nodes_edges is None:
        print(wkt_list, im_file)

    node_loc_dic, edge_dic = nodes_edges
    t1 = time.time()
    ...
    # print ("\n 0 wtk_to_G():", edge_tmp, G0[edge_tmp[0]][edge_tmp[1]])
    ...
    # for edge_tmp in G0.edges():
    # print ("\n CELANED:", edge_tmp, G0[edge_tmp[0]][edge_tmp[1]])
    ...
    if simplify_graph:
        ...
        #     print("\n G:", edge_tmp, Gout[edge_tmp[0]][edge_tmp[1]])

        # print("\n G:", edge_tmp, dict_edge)
        G0 = ox.simplify_graph(Gout.to_directed(), strict=True)

        # for edge_tmp in G0.edges():
        #     edge_start = dict(G0[edge_tmp[0]][edge_tmp[1]])[0]['start_loc_pix']
        #     edge_end = dict(G0[edge_tmp[0]][edge_tmp[1]])[0]['end_loc_pix']
        #     print('Gout', edge_start, edge_end)

        Gout = G0.to_undirected()
        # Gout = ox.project_graph(G0)
        # for edge_tmp in Gout.edges():
        #     edge_start = dict(Gout[edge_tmp[0]][edge_tmp[1]])[0]['start_loc_pix']
        #     edge_end = dict(Gout[edge_tmp[0]][edge_tmp[1]])[0]['end_loc_pix']
        #     print('Gout', edge_start, edge_end)
...
def process_img(image_id, config, graph_dir, path_images, df_wkt, pickle_protocol, min_spur_length_m,
                min_subgraph_length_pix,
                super_verbose, verbose, save_shapefiles, make_plots):
    # if image_id != 'AOI_2_Vegas_img586':
    #    continue
    out_file = os.path.join(graph_dir, image_id.split('.')[0] + '.gpickle')

    logger1.info("\n{x} / {y}, {z}".format(x=1, y=1, z=image_id))
    # print ("\n")
    ...
    # print a few values
    logger1.info("\n{x} / {y}, num linestrings: {z}".format(x=1, y=1, z=len(wkt_list)))
    # print ("\n", i, "/", len(image_ids), "num linestrings:", len(wkt_list))
    if verbose:
        print("image_file:", im_file)
        print("  wkt_list[:2]", wkt_list[:2])

    if (len(wkt_list) == 0) or (wkt_list[0] == 'LINESTRING EMPTY') or wkt_list is None:
        G = nx.MultiDiGraph()
