...
def lovasz_loss(logits, labels, error_func, per_image=True, ignore=None):
    ...
    if per_image:
        loss = mean(
            lovasz_loss_flat(
                *flatten_binary_scores(log.unsqueeze(0), lab.unsqueeze(0), ignore), error_func=error_func
...
def lovasz_loss_ignore_empty(logits, labels, truth_image, ignore=None):
    loss = mean_ignore_empty(
        (
            lovasz_loss_flat(*flatten_binary_scores(log.unsqueeze(0), lab.unsqueeze(0), ignore), error_func=hinge)
            for log, lab in zip(logits, labels)
        ),
        truth_image,
    )
