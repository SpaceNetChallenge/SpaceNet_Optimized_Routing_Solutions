    ...
    'resnext101':
        {
            ...
            'decoder_filters': [64, 128, 256, 384],
            'last_upsample': 64,
            'init_op': resnext.resnext101_32x8d_wsl,
            'url': None,
        },
}
...
class EncoderDecoder(AbstractModel):
    def __init__(self, num_classes, num_channels=3, encoder_name='resnet34'):
        ...
        if encoder_name == 'senet154' or encoder_name == 'seresnext50':
            encoder = encoder_params[encoder_name]['init_op'](pretrained='imagenet')
        else:
...
class ResnetDropout(EncoderDecoder):
    def __init__(self, seg_classes, backbone_arch):
        self.first_layer_stride_two = True
        super().__init__(seg_classes, 4, backbone_arch)
...
class EfficientNetUnet(EncoderDecoder):
    def __init__(self, seg_classes, backbone_arch):
        self.first_layer_stride_two = True
        super().__init__(seg_classes, 4, backbone_arch)
...
setattr(sys.modules[__name__], 'resnet_unet_dropout', partial(ResnetDropout))
...
setattr(sys.modules[__name__], 'scse_unet_dropout', partial(SCSEUnetDropout))
setattr(sys.modules[__name__], 'effnet_unet', partial(EfficientNetUnet))

if __name__ == '__main__':
    d = SCSEUnet(4, backbone_arch='seresnext50')
    d.eval()
    import numpy as np

    with torch.no_grad():
        images = torch.from_numpy(np.zeros((16, 4, 256, 256), dtype='float32'))
        angles = torch.from_numpy(np.zeros((16, 27, 1, 1), dtype='float32'))
        i = d((images, angles))
