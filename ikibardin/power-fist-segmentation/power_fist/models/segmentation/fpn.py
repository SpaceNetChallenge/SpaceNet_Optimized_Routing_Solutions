...
class FPNSegmentation(nn.Module):
    ...
    def __init__(self, num_classes, num_filters=128, num_filters_fpn=256, pretrained=True):
        ...
        self.encoder = self.fpn.resnet  # so that power_fist could freeze the backbone
    ...
    def forward(self, x):
        ...
        final = nn.functional.upsample(final, scale_factor=4, mode="bilinear", align_corners=False)
        return final
...
def fpn_segmentation(num_classes, num_channels=3, pretrained=True):
    return FPNSegmentation(num_classes=num_classes, pretrained=True)


if __name__ == '__main__':
    model = FPNSegmentation(num_classes=12, pretrained=True)
